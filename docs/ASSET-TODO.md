...
| A7 | River loop | 🟡 | **Synthesised stand-in shipped** (SH-052, 14 s seamless) — dark rushing bed + granular babble chirps. One live emitter per water volume tracks the closest bank point to the player. A real recording (small rocky stream, close-miked, no birds) is a straight file drop: `river_loop.wav`. |
