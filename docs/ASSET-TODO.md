...
| A4 | Music stems: calm + combat loops, and 3 stingers | 🔵 | **The one thing I can't fake — but everything around it is now built.** `assets/music/SUNO-PROMPTS.md` has the six prompts, the key/tempo lock (D minor, 120 BPM — the two beds are CROSSFADED, so a key mismatch will sound broken and cannot be fixed in the mix), the loop-cutting recipe, and the exact filenames. The game already loads all six if present and falls back to `game.wav` if not, so **dropping the WAVs in is the whole job** — no code change. Send me the raw Suno WAVs if you'd rather I cut and seam the loops. |
